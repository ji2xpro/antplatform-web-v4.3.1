<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.id" :placeholder="$t('roleTable.id')" clearable style="width: 150px;" class="filter-item" maxlength="32" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.name" :placeholder="$t('roleTable.name')" clearable style="width: 150px;" class="filter-item" maxlength="32" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.keypoint" :placeholder="$t('roleTable.keypoint')" clearable style="width: 150px;" class="filter-item" maxlength="32" @keyup.enter.native="handleFilter" />
      <el-select v-model="listQuery.status" :placeholder="$t('roleTable.status')" clearable style="width: 150px" class="filter-item">
    ...
    >
      ...
      <el-table-column :label="$t('roleTable.name')" width="" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('roleTable.keypoint')" width="" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.keypoint }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('roleTable.level')" width="" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('roleTable.status')" class-name="status-col" width="100">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status!='0'" size="mini" type="success" @click="handleModifyStatus(scope.row,'0')">{{ $t('roleTable.enable') }}
          </el-button>
          <el-button v-if="scope.row.status!='1'" size="mini" @click="handleModifyStatus(scope.row,'1')">{{ $t('roleTable.disable') }}
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="roleForm" :model="roleForm" :rules="rules" label-width="100px" class="roleForm" style="width: 400px; margin-left:50px;">
        <el-form-item :label="$t('roleTable.name')" prop="name">
          <el-input v-model="roleForm.name" placeholder="输入角色名称" maxlength="32" :clearable="true" />
        </el-form-item>
        <el-form-item :label="$t('roleTable.keypoint')" prop="keypoint">
          <el-input v-model="roleForm.keypoint" placeholder="输入角色标识" maxlength="32" :clearable="true" />
        </el-form-item>
        <el-form-item :label="$t('roleTable.level')" prop="level">
          <el-input v-model="roleForm.type" placeholder="输入角色级别" maxlength="5" :clearable="true" />
        </el-form-item>
        <el-form-item :label="$t('roleTable.status')" prop="status">
        ...
        <el-form-item :label="$t('roleTable.description')">
          <el-input v-model="roleForm.remark" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="请输入备注信息" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      statusOption: [{ label: '启用', key: '0' }, { label: '禁用', key: '1' }],
  ...
  methods: {
    ...
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        ...
        this.total = (response.paging || {}).totalHits || this.total
    ...
    handleModifyStatus(row, status) {
      this.listLoading = true
      updateRoleStatus(row.id, status).then((result) => {
        this.listLoading = false
        if (result.code === 20000) {
          row.status = status
          this.$message({
    ...
    handleUpdateResource(row) {
      ...
      getAuthList(this.treeQuery).then(response => {
        ...
        //         this.resourceTreeCheckedKeys[index] = response.data[i].id
        //         this.resourceTreeExpandedKeys[index] = response.data[i].id
        //         index++
        //       }
        //       // this.resourceTreeCheckedKeys[i] = response.data[i].permissionId
        //       // this.resourceTreeExpandedKeys[i] = response.data[i].permissionId
        //       this.oldResourceList[i] = response.data[i].id
        //     }
        //     console.log('oldResourceList:' + this.oldResourceList)
        ...
        queryRoleAuth(row.id).then(response => {
          ...
          if (auths && auths.length > 0) {
            ...
            this.oldResourceList = auths.map(item => item.id)

            console.log('oldResourceList:' + this.oldResourceList)

            // for(const auth of auths){
            //   if(auth.type === 2){
            //     this.resourceTreeCheckedKeys[index] = auth.id
            //     this.resourceTreeExpandedKeys[index] = auth.id
            //     index++
            //   }
            //   this.oldResourceList.push(auth.id)
            // }
            this.setCheckedKeys(this.resourceTreeCheckedKeys)
          }
          this.listLoading = false
    ...
    updateRoleResource() {
      ...
      // console.log('getCheckedKeys:' + keysChecked)
      // var cLength = 0
      ...
      console.log('addResourceIds:' + addResourceIds)

      var delResourceIds = that.oldResourceList.filter(function(v) { return ids.indexOf(v) === -1 })
      console.log('delResourceIds:' + delResourceIds)

      // if (addResourceIds && addResourceIds.length > 0) {
      //   for (var k = 0; k < addResourceIds.length; k++) {
      //     this.resourceData.addResources[k] = { roleId: this.currentRole, authId: addResourceIds[k] }
      //   }
      // }
      // if (delResourceIds && delResourceIds.length > 0) {
      //   for (var q = 0; q < delResourceIds.length; q++) {
      //     this.resourceData.delResources[q] = { roleId: this.currentRole, authId: delResourceIds[q] }
      //   }
      // }
